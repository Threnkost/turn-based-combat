...
func attackToEntity(target : Battler):
	...
	tween.interpolate_property(
		self, 'global_position', battler.startPosition, targetPosition, 1, Tween.TRANS_QUAD, Tween.EASE_OUT)
	tween.start()
	
func returnToStart():
	tween.interpolate_property(
		self, 'global_position', global_position, battler.startPosition, 1, Tween.TRANS_QUAD, Tween.EASE_OUT)
	tween.start()
